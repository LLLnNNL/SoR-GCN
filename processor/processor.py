...
class Processor():
    ...
    @torch.no_grad()
    def debug_check_R_once(self, loader_name='test', OBR=40):

        self.model_encoder.eval()
        if isinstance(self.model_encoder, torch.nn.DataParallel):
            encoder = self.model_encoder.module
        else:
            encoder = self.model_encoder
        device = torch.device(f'cuda:{self.output_device}'
                              if torch.cuda.is_available() else 'cpu')
        encoder.to(device)

        mods = encoder.get_L1_L10_ctrgc_modules()
        print("[info] CTRGC modules in L1 & L10:")
        ...
        any_loader = None
        for k, loader in self.data_loader[loader_name].items():
            any_loader = loader
            if hasattr(any_loader.dataset, 'set_OBR'):
                any_loader.dataset.set_OBR(OBR)
                print(f"[info] use loader='{k}', set OBR={OBR}")
            break

        if any_loader is None:
            print(f"[error] data_loader['{loader_name}'] 为空")
            return

        data, label, index = next(iter(any_loader))
        # data: (N, M, T, V, C)
        print("[info] data shape:", data.shape)
        print("[info] first batch labels (raw):", label[:16])
        print("[info] label min:", label.min().item(), "label max:", label.max().item())
        ...
        for name, m in mods.items():
            R = m.last_R
            if R is None:
                print(f"[warn] {name}.last_R is None (可能 save_R 没生效)")
                continue
            print(f"[info] {name}: R.shape = {R.shape}")

            print(f"       {name}: R.min={R.min().item():.4f}, R.max={R.max().item():.4f}")

        for m in mods.values():
            m.save_R = False

        print("[info] debug_check_R_once done.")

...
def get_parser():
    ...
    parser.add_argument(
        '--data_path', default='./data/nturgbd_raw/nturgb+d_skeletons120')  # 导入数据集源位置
    parser.add_argument(
        '--ignored_sample_path',
        default='./data_gen/NTU120_missing.txt')  # 忽略不处理的源文件名称
    parser.add_argument('--out_folder', default='./data/NTU120')  # 处理后数据集存储位置
...
def check_samples(filename, benchmark, part):
    ...
    if benchmark == 'xset':
        istraining = (setup_id % 2 == 0)      # 偶数S训练，奇数S验证（官方协议）
    elif benchmark == 'xsub':
...
def gendata(data_path, out_path, ignored_sample_path=None, benchmark='xsub', part='val',gen_ori=False):
    ...
    for entry in os.scandir(data_path):
        if not entry.is_file():
            continue
        filename = entry.name
        if filename.startswith('.') or not filename.endswith('.skeleton'):
            continue  # 跳过 .empty / .DS_Store / 其他非 .skeleton
        if filename in ignored_samples:
            continue
